...
def Pop_Sim2(N, m, gamma):
    ...
    for i in range(N - m0):
        ...
        dist_index = choose_neighbour (G, gamma,sim_matrix, int(n[i+m0]))
        G.add_node(n[m0 + i])
        new_edges = []
        # m must not be less than m0
        for k in range (m):
            G.add_edge(n[m0+i],dist_index[k])
            new_edges.append((n[m0 + i],dist_index[k]))
            print new_edges
...
def choose_neighbour(G, gamma, sim, t):
...
def hyperbolic_dist(G, gamma,sim, t):
    ...
    for node in G.nodes():
        ...
        theta_st = abs(2 + (sim[int(node)-1][t-1] * (pi / 2)))
